...
- Вкладка использует stub API `POST /api/images/search` и пока не выполняет реальную вставку.
...
3. Переключитесь на **Свойства** — должен отображаться текущий функционал без изменений.
4. Выберите обычную картинку (не placeholder) — вкладки не показываются.
5. Импортируйте архив без `imagePlan.json` — всё работает по-старому.
