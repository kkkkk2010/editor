...
4. В DevTools проверьте запрос `POST /api/images/fetch`: поле `imageUrl` должно приходить как финальный `https://fastly.picsum.photos/...hmac=...` (валидный, не фейковый), ответ fetch должен быть `200`, и вставка не должна падать с `403/502`.
5. Проверьте, что на холсте картинка обновилась; статус показывает **Вставлено**.
6. Экспортируйте `out.zip` и убедитесь, что:
   - файл ассета по `src` заменён,
