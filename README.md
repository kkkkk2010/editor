...
4. Проверьте, что на холсте картинка обновилась; статус показывает **Вставлено**.
5. Экспортируйте `out.zip` и убедитесь, что:
   - файл ассета по `src` заменён,
   - в корне есть `imageCredits.json`.
6. Переключитесь на **Свойства** — должен отображаться текущий функционал без изменений.
7. Выберите обычную картинку (не placeholder) — вкладки не показываются.
8. Импортируйте архив без `imagePlan.json` — всё работает по-старому.
