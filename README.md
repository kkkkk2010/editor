...
### Placeholder Images в панели свойств изображения

Если в `out.zip` есть `imagePlan.json`, для image-элементов, которые совпадают по `(slideIndex+1, elementIndex)`,
внутри правой панели изображения появится вторая вкладка **Подобрать**.

- Для обычных картинок вкладки не показываются: панель выглядит как раньше.
- Для placeholder-картинок по умолчанию открывается вкладка **Подобрать**.
- Вкладка использует API `POST /api/images/search` и `POST /api/images/fetch`.
- Кнопка **Вставить** активна только после выбора результата и установки чекбокса **Я проверил права**.
- После вставки картинка сразу обновляется на холсте и попадает в экспорт `out.zip` как замена байтов по исходному `src`.
- При экспорте добавляется `imageCredits.json` в корень архива.

Manual test:

1. Импортируйте `out.zip` c `imagePlan.json`.
2. Выберите image-элемент, который отмечен в плане:
   - должны появиться вкладки **Свойства / Подобрать**,
   - по умолчанию открыта **Подобрать**,
   - отображаются `slotId`, `query`, `hint` (если есть).
3. Нажмите **Найти**, выберите результат, отметьте чекбокс и нажмите **Вставить**.
4. В DevTools проверьте запрос `POST /api/images/fetch`: для mock-выдачи поле `imageUrl` должно приходить как локальный путь `/mock-images/*`, ответ fetch должен быть `200` даже оффлайн, и вставка не должна падать из-за внешних 403/502.
5. Проверьте, что на холсте картинка обновилась; статус показывает **Вставлено**.
6. Экспортируйте `out.zip` и убедитесь, что:
   - файл ассета по `src` заменён на bytes выбранной картинки,
   - в корне есть `imageCredits.json`.
