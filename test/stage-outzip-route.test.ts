...
describe("POST /api/bridge/stage-outzip", () => {
  beforeEach(() => {
    ...
    delete process.env.BRIDGE_SAVE_TOKEN_VALIDATE_URL
    delete process.env.BRIDGE_SAVE_TOKEN_VALIDATE_BEARER
  })
  ...
  it("authorizes save flow via server-side save token validation", async () => {
    process.env.BRIDGE_SAVE_TOKEN_VALIDATE_URL = "http://wp.local/validate-save-token"
  ...
  it("rejects save fallback when validation endpoint denies token", async () => {
    process.env.BRIDGE_SAVE_TOKEN_VALIDATE_URL = "http://wp.local/validate-save-token"
    vi.spyOn(globalThis, "fetch").mockResolvedValue(new Response(JSON.stringify({ ok: false }), { status: 403 }))

    const request = makeRequest(ZIP_BYTES, { requestId: "req-save-denied" })
    request.headers.set("x-presentation-id", "123")
    request.headers.set("x-save-token", "a".repeat(48))
